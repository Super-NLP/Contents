...
* RNN, LSTM과 GRU는 언어 모델링, 기계 번역과 같은 순차 모델링 및 변환 문제에서 최고의 접근 방식으로 확고히 자리잡고 있고 이후에도 많은 연구가 순환 언어 모델과 인코더-디코더 구조의 한계를 뛰어넘기 위해 계속되고 있음.
  * 순환 모델은 일반적으로 입력 및 출력 시퀀스의 심볼 위치에 따라 계산을 분할함. 연산 steps의 위치에 따라, 이전 hidden state인 $h\_{t-1}$ 와 position $t$ 의 입력을 기능으로 하는 hidden state $h\_t$가 생성됨.
...
* 인코더의 입력 시퀀스 $(x\_1,\cdots,x\_n)$ 를 연속적 표현인 $z = (z\_1,\cdots,z\_n)$와 같이 매핑하여 표현함.
* $z$를 바탕으로 디코더는 출력 시퀀스 $(y\_1, \cdots, y\_m)$ 를 하나씩 생성함.
...
* Residual Connection을 용이하게 하기 위해, 모델의 모든 서브레이어와 임베딩 레이어는 $d\_{model}=512$의 차원을 가진 출력을 생성함.
...
* Scaled Dot-Product Attention의 입력은 $d\_k$에 대한 query, key와 $d\_v$에 대한 value의 벡터들로 구성됨.
* 모든 key로 dot-product를 계산하고 각각을 $\sqrt{d\_k}$로 나눈 다음, Softmax함수를 적용하여 value의 가중치를 얻게됨.
* 가장 일반적으로 사용되는 attention function은 'additive attention'과 'dot-product attention'이고 이중 Dot-product attention의 스케일링 인자인 $\frac{1}{\sqrt{d\_k\}}$를 제외하면 본 논문의 알고리즘과 같음.
...
* 위의 두 가지는 이론적인 복잡성은 유사하지만, dot-product attention이 훨씬 빠르고 공간효율적임.
  * $d\_k$가 작으면 두 방식의 성능은 비슷하지만, $d\_k$가 큰 경우 additie attention이 더 성능이 좋음.
  * $d\_k$가 크면 dot-product의 경우 gradient vanishing 문제를 해결하기 위해 $\frac{1}{\sqrt{d\_k\}}$로 스케일링 함.

#### **3.2.2 Multi-Head Attention**

![](https://velog.velcdn.com/images/ash-hun/post/48e2c2d4-82f9-4263-9cf0-e2b5d3b8040d/image.png)

* $d\_{model}$의 차원크기를 가지는 key, value, query로 단일 어텐션 기능을 수행하는 대신에 query, key,value을 각각 $d\_k$, $d\_k$, $d\_v$ 차원으로 $h$번 linear projection하는 것(=> 서로 다르게 학습)이 더 효과적이라는 것을 발견함.
* 이렇게 projection된 query, key, value에 대해서 attention function을 병렬적으로 수행하여 $h$개의 $d\_v$ 차원의 결과를 산출함. 이 결과들을 연결시킨 후 linear projection하여 최종적인 결과벡터를 얻게됨.

![](https://velog.velcdn.com/images/ash-hun/post/fadd3423-9447-448b-80a1-7b4cf53eb04d/image.png)

* 파라미터들
  * $W^Q\_i \in \mathbb{R}^{d\_{model} \times d\_k}$
  * $W^K\_i \in \mathbb{R}^{d\_{model} \times d\_k}$
  * $W^V\_i \in \mathbb{R}^{d\_{model} \times d\_v}$
  * $W^O\_i \in \mathbb{R}^{hd\_v \times d\_{model\}}$
  * 본 연구에선 $h=8$이고 $d\_k=d\_v=d\_{model}/h=64$이다. 각 head마다 차원을 줄이기 때문에 전체계산비용이 단일 어텐션과 비슷하다.
...
* 선형변환은 position에 대해서는 동일하지만, 각 층마다 다른 파라미터 $(W\_1, W\_2)$를 사용함.
* 입출력 차원은 동일하게 $d\_{model}=512$.
* FFN 내부 hidden layer의 차원은 $d\_{FFN}=2018$.

### 3.4 Embedding and Softmaxs

* 다른 시퀀스 변환 모델과 유사하게, 학습된 임베딩을 사용함.
  * 입력 토큰과 출력 토큰을 $d\_{model}$차원의 벡터로 변환하기 위함.
...
* 임베딩 레이어에서는 그 가중치에 $\sqrt{d\_{model\}}$을 곱해줌.
...
* Positional Encoding은 input 임베딩과 마찬가지로, $d\_{model}$ 차원을 가지기 때문에 합칠 수 있음.
...
* 파장은 $2\pi$에서 $10000 \* 2\pi$까지 기하급수적으로 늘어남.
* 위의 함수를 선택한 이유는 어떤 고정된 오프셋 $k$에 대해,$PE\_{pos+k}$가 $PE\_{pos}$의 선형함수로 표현될 것이라는 가설때문. (상대적으로 위치를 쉽게 학습할 것이다!)
...
* Residual Dropout
  ...
  * $$P_{drop}=0.1$$
* Label Smoothing
  * 훈련하는동안 label smoothing value $\epsilon\_{ls}=0.1$을 사용함.
